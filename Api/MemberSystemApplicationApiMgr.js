...
// 查健身工廠會員系統各類別一年內最近一筆申請（最多 9 類）。Product/development → 呼叫客戶 API；
// Qbi → 讀本機 data/ mock。
// TODO(客戶提供)：客戶尚未提供本查詢 API 規格，Qbi 先行開發，待客戶 API 到位後對照欄位調整 normalize()。
class MemberSystemApplicationApiMgr {
    async query({ chatId, key, logger }) {
        const cfg = (ExternalConfig.MemberSystemApplicationQuery && ExternalConfig.MemberSystemApplicationQuery[ExternalConfig.Mode]) || {};
        ...
        if (ExternalConfig.Mode === 'Qbi') {
            ...
            logger && logger.InfoLog(`[MemberSystemApplicationApiMgr] → 讀本機 mock: ${filePath}`);
            source = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            logger && logger.InfoLog(`[MemberSystemApplicationApiMgr] ← mock 回傳（${Date.now() - started}ms）Body: ${JSON.stringify(source)}`);
        } else {
            const body = { name: 'MemberSystemApplicationQuery', from: 'csr', sessionId: chatId || CommonMethod.makeSessionId(), formData: { key } };
            logger && logger.InfoLog(`[MemberSystemApplicationApiMgr] → 呼叫 API: ${cfg.Url}`);
            logger && logger.InfoLog(`[MemberSystemApplicationApiMgr] → Request Body: ${JSON.stringify(body)}`);
            const resp = await axios.post(cfg.Url, body, {
                timeout: ExternalConfig.RequestTimeout,
                headers: { 'Content-Type': 'application/json' }
            });
            logger && logger.InfoLog(`[MemberSystemApplicationApiMgr] ← 回傳（${Date.now() - started}ms, HTTP ${resp.status}）Body: ${JSON.stringify(resp.data)}`);
            source = this.parseSource(resp.data);
        }
        const result = this.normalize(source, key);
        logger && logger.InfoLog(`[MemberSystemApplicationApiMgr] 解析結果: found=${result.found}`);
        return result;
    }
...
module.exports = new MemberSystemApplicationApiMgr();
