...
// 查教練合約異動申辦進度（課程轉讓／課程終止／更換教練）。Product/development → 呼叫客戶 API；
// Qbi → 讀本機 data/ mock。
// TODO(客戶提供)：客戶尚未提供本查詢 API 規格，Qbi 先行開發，待客戶 API 到位後對照欄位調整 normalize()。
class CoachApplicationApiMgr {
    async query({ chatId, key, logger }) {
        const cfg = (ExternalConfig.CoachApplicationQuery && ExternalConfig.CoachApplicationQuery[ExternalConfig.Mode]) || {};
        ...
        if (ExternalConfig.Mode === 'Qbi') {
            ...
            logger && logger.InfoLog(`[CoachApplicationApiMgr] → 讀本機 mock: ${filePath}`);
            source = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            logger && logger.InfoLog(`[CoachApplicationApiMgr] ← mock 回傳（${Date.now() - started}ms）Body: ${JSON.stringify(source)}`);
        } else {
            const body = { name: 'CoachApplicationQuery', from: 'csr', sessionId: chatId || CommonMethod.makeSessionId(), formData: { key } };
            logger && logger.InfoLog(`[CoachApplicationApiMgr] → 呼叫 API: ${cfg.Url}`);
            logger && logger.InfoLog(`[CoachApplicationApiMgr] → Request Body: ${JSON.stringify(body)}`);
            const resp = await axios.post(cfg.Url, body, {
                timeout: ExternalConfig.RequestTimeout,
                headers: { 'Content-Type': 'application/json' }
            });
            logger && logger.InfoLog(`[CoachApplicationApiMgr] ← 回傳（${Date.now() - started}ms, HTTP ${resp.status}）Body: ${JSON.stringify(resp.data)}`);
            source = this.parseSource(resp.data);
        }
        const result = this.normalize(source, key);
        logger && logger.InfoLog(`[CoachApplicationApiMgr] 解析結果: found=${result.found}`);
        return result;
    }
...
module.exports = new CoachApplicationApiMgr();
