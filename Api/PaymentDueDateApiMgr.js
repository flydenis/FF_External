...
// 查月費扣款日。Product/development → 呼叫客戶 API；Qbi → 讀本機 data/ mock。
// TODO(PM 確認)：query() 目前用 key 當查詢條件的預留欄位，實際 request 要帶什麼欄位（會員編號/合約編號對應識別碼）待客戶 API 規格確定後調整。
class PaymentDueDateApiMgr {
    async query({ chatId, key, logger }) {
        const cfg = (ExternalConfig.PaymentDueDateQuery && ExternalConfig.PaymentDueDateQuery[ExternalConfig.Mode]) || {};
        ...
        if (ExternalConfig.Mode === 'Qbi') {
            ...
            logger && logger.InfoLog(`[PaymentDueDateApiMgr] → 讀本機 mock: ${filePath}`);
            source = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            logger && logger.InfoLog(`[PaymentDueDateApiMgr] ← mock 回傳（${Date.now() - started}ms）Body: ${JSON.stringify(source)}`);
        } else {
            const body = { name: 'PaymentDueDateQuery', from: 'csr', sessionId: chatId || CommonMethod.makeSessionId(), formData: { key } };
            logger && logger.InfoLog(`[PaymentDueDateApiMgr] → 呼叫 API: ${cfg.Url}`);
            logger && logger.InfoLog(`[PaymentDueDateApiMgr] → Request Body: ${JSON.stringify(body)}`);
            const resp = await axios.post(cfg.Url, body, {
                timeout: ExternalConfig.RequestTimeout,
                headers: { 'Content-Type': 'application/json' }
            });
            logger && logger.InfoLog(`[PaymentDueDateApiMgr] ← 回傳（${Date.now() - started}ms, HTTP ${resp.status}）Body: ${JSON.stringify(resp.data)}`);
            source = this.parseSource(resp.data);
        }
        const result = this.normalize(source, key);
        logger && logger.InfoLog(`[PaymentDueDateApiMgr] 解析結果: found=${result.found}`);
        return result;
    }
...
module.exports = new PaymentDueDateApiMgr();
